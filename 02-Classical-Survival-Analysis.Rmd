...
Suppose we are taking survival data from two different groups. This might be a treatment and a control group. We would like to be able to say whether or not the groups seem distinct. A non-parametric approach would be to use the Log-Rank test. In this test, at each time step we look at new infections in each group compared to the total healthy population in the time before. We also look at the two groups together. We can compute the $\chi^{2}$ statistic by calculating

$$
\chi^{2} = \frac{\left(E_{A} - O_{A}\right)^{2}}{E_{A}} + \frac{\left(E_{B} - O_{B}\right)^{2}}{E_{B}}
$$

For a single timestep like this, we have one degree of freedom. We can use the $\chi^{2}$ test statistic in order to compute the liklihood that both groups come from the same distribution. (In other words, the treatment is ineffective.) We can calculate a $p$ value by comparing the two groups, and reject the null hypothesis (that the two groups are equivalent) if $p < 0.05$.

# Kaplan Meier analysis of the plant data
...
Trial_list <- c("Test1LL", "Test2LL", "Test3LL", "Test4LL")
...
for (eco in Eco_list){
  for (trial in Trial_list) {
    ...
    print(ggsurvplot(fit, data = temp_df, conf.int = TRUE, pval = TRUE, theme = theme_cowplot()))
  }
}
